...
export const DataCard: React.FC<DataCardProps> = ({
  ...
  // Calcular suggestions només quan hi ha query i no hi ha selecció
  const suggestions: CodeBookEntry[] = (searchQuery.trim() && !selectedEntry)
                ...
                {suggestions.length > 0 && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
                    {suggestions.map((suggestion, idx) => (
                      <button
                        key={`${suggestion.Codi}-${idx}`}
                        onClick={() => handleSelectCode(suggestion)}
                        className="w-full text-left px-4 py-3 hover:bg-blue-50 text-sm border-b border-gray-100 last:border-0 transition"
                      >
